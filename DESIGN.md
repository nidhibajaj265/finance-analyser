...
| **13. GitHub Actions deployment** | Write `.github/workflows/daily_run.yml` to run the full pipeline on a schedule (once daily); handle secrets (HF_TOKEN, OPENAI_API_KEY), NLTK data download, and artifact storage for outputs | Pipeline runs automatically on a cron schedule with no manual trigger. |
